...
import com.mycompany.negozio.Abito;
...
public class Ordinatore 
{
     ...
     * @param pos2 
     */
    public static void scambiaAbiti(Abito[] a, int pos1, int pos2)
    {
        Abito s;
        s=a[pos1];
        a[pos1]=a[pos2];
     ...
     * @return  una copia dell'array ordinato
     */
    public static Abito[] selectionSortCrescenteAbiti (Abito[] a)
    {
        Abito v[]= new Abito[a.length];
        int lunghezza=v.length;
        //creo una copia di a
        for(int i=0;i<v.length;i++)
        {
        ...
        for (int i=0;i<lunghezza-1;i++)
        {
            for(int j=i+1;j<lunghezza;j++)
            {
                if (v[j].getMarca().compareToIgnoreCase(v[i].getMarca())<0)
                    scambiaAbiti(v, i, j);
            }
        }       
        return v;    
